...
export async function getAdminAnalytics(): Promise<AdminAnalytics> {
  try {
    ...
    return {
      ...
      bookingsByStatus,
      bookingsByMonth,
      topCategories: Array.from(categoryMap.entries()).map(
        ([category, count]) => ({
          category,
          count,
...
export async function getProfileStats(userId: string) {
  try {
    const [
      itemsResult,
      bookingsAsOwnerResult,
      bookingsAsRenterResult,
      reviewsResult,
    ] = await Promise.all([
  ...
  try {
    ...
    if (booking.status === "confirmed" && isRenter) {
      ...
      if (hoursUntilStart < 24) {
        return {
          success: false,
          message: "Cannot cancel within 24 hours of the rental start time",
        };
      }
    }
    ...
    if (updateError) {
      ...
      return {
        success: false,
        message: "Failed to cancel booking: " + updateError.message,
      };
    }
    ...
    if (detailsError) {
      ...
      const notificationUserId = isRenter
        ? booking.owner_id
        : booking.renter_id;
      const notificationTitle = isRenter
        ? "Booking Cancelled by Renter"
        : "Booking Cancelled by Owner";
      const notificationMessage = isRenter
        ? `${
            bookingWithDetails.renter?.full_name || "A renter"
          } has cancelled their booking for "${
            bookingWithDetails.item?.name || "an item"
          }"`
        : `${
            bookingWithDetails.owner?.full_name || "The owner"
          } has cancelled the booking for "${
            bookingWithDetails.item?.name || "an item"
          }"`;
  ...
  try {
    ...
    if (booking.status === "confirmed" && isRenter) {
      ...
      if (hoursUntilStart < 24) {
        return {
          canCancel: false,
          reason: "Cannot cancel within 24 hours of start time",
        };
      }
    }
